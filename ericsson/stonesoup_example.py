...
from stonesoup.feeder.time import (  # noqa: F401
    TimeBufferedFeeder,
    TimeSyncFeeder,
)  # (optional wrappers)
...
class SensorCfg:
    ...
    pos_noise_std: float
    phase: float = 0.0
    start: dt.datetime | None = None  # if None, set at init
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        ...
        while not self._stop.is_set():
            ...
            for x0, y0, vx, amp, omega, phase in params:
                ...
                meas = np.array([x, y]) + np.random.normal(
                    0.0, self.cfg.pos_noise_std, 2
                )
...
def run(runtime_s: float = 12.0):
    ...
    feeder = build_fifo_feeder(
        [r1, r2], max_queue=256, time_ordering=False, time_window_s=None
    )
